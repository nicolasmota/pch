...
from pch_core.errors import ReconsentRequired, ValidationFailed
from pch_core.policy.plugin import manifest_from_snapshot, permission_diff
from pch_core.service import Hub
from pydantic import BaseModel

from pch_server.marketplace.catalog import find_listing, load_cached_catalog, refresh_catalog
from pch_server.plugins.host import isolation_mode
from pch_server.plugins.package import manifest_to_snapshot, sha256_bytes, unpack_package
...
def get_catalog(hub: Hub = Depends(get_hub), _: str = Depends(require_owner)) -> dict:
    from pch_server.marketplace.catalog import catalog_url, local_bundled_catalog
...
def install(body: InstallBody, hub: Hub = Depends(get_hub), _: str = Depends(require_owner)) -> dict:
    ...
    if listing.get("origin") == "bundled" or not package_url:
        ...
        from pch_server.plugins.paths import bundled_dir
    ...
    if sha256_bytes(response.content) != expected:
        from pch_core.errors import IntegrityMismatch
