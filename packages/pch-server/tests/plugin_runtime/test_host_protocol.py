...
GOOD = '''from pch_sdk.plugin_runtime import hub
...
DENY_TYPE = '''from pch_sdk.plugin_runtime import hub
...
MANY = '''from pch_sdk.plugin_runtime import hub
...
@pytest.mark.plugins
def test_plugin_env_does_not_inject_checkout_sdk_src(tmp_path):
    from pch_server.plugins.host import plugin_child_env

    env = plugin_child_env(tmp_path, {"manifest": {}})
    injected = (env.get("PYTHONPATH") or "").replace("\\", "/")
    assert "packages/pch-sdk/src" not in injected
