...
from pcl_core.errors import Busy, ConsentRequired, ValidationFailed
...
from pcl_server.plugins.protocol import API_VERSION, ProtocolError, decode_line, dispatch, encode
...
def sandbox_available() -> bool:
    ...
    try:
        result = subprocess.run(
            [bwrap, "--unshare-net", "--die-with-parent", "/bin/true"],
            capture_output=True,
            timeout=2,
        )
...
def _spawn(root: Path, inst: dict) -> subprocess.Popen:
    ...
    sdk_src = Path(__file__).resolve().parents[4] / "pcl-sdk" / "src"
    env["PYTHONPATH"] = os.pathsep.join(
        p for p in [str(sdk_src), env.get("PYTHONPATH", "")] if p
    )
    cmd = [sys.executable, "-c", "from pcl_sdk.plugin_runtime import serve; serve()"]
    if shutil.which("bwrap") and inst.get("isolation") == "sandboxed":
        cmd = [
            "bwrap",
            "--unshare-net",
            "--die-with-parent",
