...
Plugin code talks to the Hub through `pcl_sdk.plugin_runtime` (stdio JSON-RPC). It never sees the vault key or the open network — `hub.http.fetch` is host-mediated against declared hostnames.
...
The Hub UI has **Plugins** and **Marketplace**. Marketplace listings are a signed static `catalog.json`. Third-party publish is a curator-reviewed PR, not self-service.

## Developer loop

```bash
uv run pcl-sdk plugin new my.importer
uv run pcl-sdk plugin validate my.importer
uv run pcl-sdk plugin pack my.importer
# Side-load the .pclplugin via Plugins, or POST /v1/plugins with source=sideload
uv run pcl-sdk plugin dev my.importer --hub http://127.0.0.1:8765
```
...
Imports from the plugin tree are limited: stdlib allowlist plus `pcl_sdk`. Importing `pcl_core` / `pcl_server` fails `plugin validate`.
