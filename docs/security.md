...
Policy is evaluated in `pcl-core`, outside any model’s reasoning.

## Loopback only

`pch`, `pcl-server`, and `hub-desktop` refuse a non-loopback `--host` (exit code 2). The message is: *The Hub is not a public server; it binds loopback only.*
...
`pch doctor` reports whether encryption is on. From-source `pcl-server` may open a plain database when the driver is missing and `plain=True` is used in tests — do not treat that as the product default.
