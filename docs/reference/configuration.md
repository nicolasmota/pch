...
Node.js 22+ is **build-time** for the UI. Runtime is Python. `make frontend` runs `npm ci` and `npm run build` into `pcl-server` static assets (gitignored). `make serve` / `make desktop` run `npm run build:watch` alongside the API.
...
`make check-secrets` fails the PR if tracked paths match. Speckit packs under `specs/` are gitignored and must not be published.
