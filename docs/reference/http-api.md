...
`Hub.ask()` exists in `pcl-core` but is **not** exposed as `/v1/ask`.
...
When `PCH_SIM_ENABLED=1` or the app is created with `sim_enabled=True` (`make serve`, `hub-desktop --dev`, `pch serve --sim`):
