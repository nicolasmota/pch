...
def test_dev_defaults_reload_not_sim(monkeypatch, tmp_path: Path):
    captured: dict = {}
    monkeypatch.setenv("PCH_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("PCH_PLAIN_SQLITE", "1")
    monkeypatch.delenv("PCH_SIM_ENABLED", raising=False)
    monkeypatch.delenv("PCH_CATALOG_REFRESH", raising=False)

    def fake_run_server(host, port, reload=False):
        captured["reload"] = reload

    monkeypatch.setattr("hub_desktop.main.run_server", fake_run_server)
    from hub_desktop.main import main as desktop_main

    desktop_main(["--dev", "--data-dir", str(tmp_path)])
