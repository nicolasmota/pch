...
from hub_desktop.pin import ensure_pinned, pinned_interpreter, reexec_into_pinned
...
def test_first_launch_does_not_hit_pypi(tmp_path: Path, monkeypatch, capsys):
    ...
    assert action == "skipped_no_pypi"
    assert not log.exists() or "tool install" not in log.read_text()
    assert "personal-context-hub==" not in capsys.readouterr().out
...
def test_missing_uv_still_launches_from_checkout(monkeypatch, tmp_path: Path):
    monkeypatch.setattr("hub_desktop.pin.shutil.which", lambda name: None)
    monkeypatch.setattr("hub_desktop.pin.running_from_uv_tool", lambda: False)
    action = ensure_pinned(__version__)
    assert action == "skipped_no_pypi"
    called = {}
    monkeypatch.setattr("hub_desktop.cli.ensure_pinned", lambda v: "skipped_no_pypi")
...
def test_upgrade_refuses_without_pypi(capsys) -> None:
    from hub_desktop.uninstall import NO_PYPI_UPGRADE, upgrade

    assert upgrade() == 2
    assert "no pypi release" in capsys.readouterr().out.lower()
    assert "make install" in NO_PYPI_UPGRADE
