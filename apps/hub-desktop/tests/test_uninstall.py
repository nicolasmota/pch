...
def test_default_keeps_data(tmp_path: Path, monkeypatch, capsys):
    ...
    vault.write_text("x")
    called: list = []
    monkeypatch.setattr("hub_desktop.uninstall.running_from_uv_tool", lambda: False)
    monkeypatch.setattr(
        "hub_desktop.uninstall.run_uv_tool",
    ...
    assert "uv tool" in out.lower() or "checkout" in out.lower()


def test_uv_tool_uninstall_when_running_from_tool(tmp_path: Path, monkeypatch):
    called: list = []
    ...
    assert uninstall(tmp_path, purge_data=False, yes=False) == 0
    assert called == [["tool", "uninstall", "personal-context-hub"]]
