...
from pch_core.vault.engine import cipher_available
...
def cmd_capture(args: argparse.Namespace) -> None:
    ...
    require_cipher_or_exit(data_dir)
    statement = " ".join(getattr(args, "statement", None) or []).strip()
    title = getattr(args, "title", None)
    if not statement:
        print("capture needs a durable fact")
        raise SystemExit(1)
    plain = os.environ.get("PCH_PLAIN_SQLITE") == "1"
    hub = Hub(data_dir, plain=plain)
    try:
        if not hub.setup_status().get("initialized"):
            print("capture needs setup first")
            raise SystemExit(1)
        result = hub.owner_capture(statement, title=title)
        print(f"Captured into {result['project'].get('title') or 'the live situation'}")
...
def _parser() -> argparse.ArgumentParser:
    ...
    capture.add_argument("--data-dir", default=_data_dir_default)
    capture.add_argument("--title")
    capture.add_argument("statement", nargs="*")
