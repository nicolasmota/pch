...
from pch_server.plugins.sandbox import isolation_mode, sandbox_backend
...
def ui_bundled() -> bool:
    return (Path(pch_server.__file__).resolve().parent / "static" / "index.html").is_file()
...
def doctor_report(data_dir: Path, port: int = 8765) -> dict:
    ...
    return {
        ...
        "port": chosen,
        "plugin_isolation": isolation_mode(),
        "plugin_sandbox_backend": sandbox_backend(),
    }
