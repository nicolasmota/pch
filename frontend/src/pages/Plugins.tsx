...
export default function Plugins() {
  ...
  async function syncNow(id: string) {
    ...
    try {
      ...
      await refresh();
      setMsg(
        `Sync ${summary.outcome || "ok"}: ${summary.created ?? 0} created, ${summary.updated ?? 0} updated.`,
      );
    } catch (err) {
      setError(errorMessage(err));
    } finally {
  ...
  return (
    <section className="space-y-6">
      ...
      <p className="text-sm text-muted">
        ...
        <Link to="/marketplace" className="text-ink underline">
          Marketplace
        </Link>
        , off the main menu.
      </p>
      ...
      {items.map((item) => (
        <Card key={item.id || item.plugin_id} className="space-y-2">
          ...
          {item.last_run?.at ? (
            <p className="text-sm text-muted">
              Last run: {item.last_run.at} ({item.last_run.outcome}) · {item.last_run.created ?? 0}{" "}
              created, {item.last_run.updated ?? 0} updated
            </p>
          ) : null}
          <div className="flex flex-wrap gap-2">
            ...
            {item.state === "enabled" && item.id ? (
              <>
                ...
                <Button
                  variant="secondary"
                  onClick={() => void lifecycle(item.id as string, "pause")}
                  disabled={busy}
                >
                  Pause
                </Button>
            ...
            {item.state === "paused" && item.id ? (
              <Button
                variant="secondary"
                onClick={() => void lifecycle(item.id as string, "enable")}
                disabled={busy}
              >
                Resume
              </Button>
