...
export default function Marketplace() {
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        title="Marketplace"
        description="Signed catalog. Not in the main menu — open it from Plugins or this URL. Install still requires the same consent screen."
      />
      {catalog?.source === "bundled" ? (
        <Alert tone="info">
          Showing first-party plugins shipped with this Hub. A remote catalog is optional.
        </Alert>
      ) : null}
      {catalog?.offline ? (
        <Alert tone="info">
          Remote catalog is unreachable. Showing a cached or empty list. Installed plugins keep
          working.
        </Alert>
      ) : null}
      ...
      {(catalog?.listings || []).map((listing) => {
        ...
        return (
          <Card key={listing.plugin_id} className="space-y-2">
            ...
            {perms?.hosts ? (
              <p className="text-sm text-muted">Hosts: {perms.hosts.join(", ")}</p>
            ) : null}
            {listing.withdrawn ? (
              <Alert tone="error">
                Withdrawn: {listing.withdrawn.reason || "removed from catalog"}
              </Alert>
            ) : listing.installed ? (
              <p className="text-sm text-muted">
                Already installed{listing.installed_state ? ` · ${listing.installed_state}` : ""}.
                Manage it in Plugins.
              </p>
            ) : (
