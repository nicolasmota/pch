...
```bash
uv run pcl-sdk loop start --mode full --desc "…"
uv run pcl-sdk loop start --mode full --epic E3
uv run pcl-sdk loop start --mode full --dir specs/005-temporal-validity --resume
uv run pcl-sdk loop start --mode full --roadmap
uv run pcl-sdk loop start --mode design-only --dir specs/… --resume
```

Then repeat until `loop status` shows `complete`, `failed`, `stopped`, or `blocked_on_person`:

1. `uv run pcl-sdk loop next`
2. Execute the named skill/stage (table below)
3. `uv run pcl-sdk loop record --stage <id> --outcome pass|fail|blocked_on_person`

Never implement `docs/VISION.md` or `docs/ROADMAP.md`. Never reopen 001–003 as epics. Never skip Gauntlet. Never commit unless the person asked.
...
| `make-test` | Run `make test` and `make lint` as appropriate; `uv run pcl-sdk loop evidence --command "make test" --exit-code N --summary "…"` then record `test` |
...
```bash
uv run pcl-sdk loop record-verdict --critic A --verdict WIN|LOSE --round N [--failing 1,2]
uv run pcl-sdk loop record-verdict --critic B --verdict WIN|LOSE --round N [--failing 1,2]
```
...
`uv run pcl-sdk loop status` — show this instead of asking the person what ran.

`uv run pcl-sdk loop stop` — person asked to stop.
...
Loopback only. Not a Hub client. No vault writes. `pcl-core` stays free of I/O from this skill.
