...
def check(dist: Path) -> list[str]:
    ...
    server = by_name["pcl_server"]
    with zipfile.ZipFile(server) as zf:
        nameset = set(zf.namelist())
        if "pcl_server/static/index.html" not in nameset:
            errors.append("pcl_server wheel missing pcl_server/static/index.html")
        if not any(
            n.startswith("pcl_server/static/assets/") and n.endswith(".js") for n in nameset
        ):
            errors.append("pcl_server wheel missing pcl_server/static/assets/*.js")
    ...
    siblings = {
        "pcl_pca": ["pcl-core"],
        "pcl_sdk": ["pcl-core"],
        "pcl_server": ["pcl-core", "pcl-pca", "pcl-sdk"],
        "personal_context_hub": ["pcl-server"],
    }
